...
export const useGetMyRestaurant = () => {
  ...
  // Real-time: refetch when any restaurant is updated via Socket.IO
  useSocketEvent("restaurant-updated", () => {
...
export const useGetMyRestaurantOrders = (
  ...
  const { data: orders, isLoading, isFetching, refetch } = useQuery(
    ...
    getMyRestaurantOrdersRequest,
    {
      ...
      refetchInterval: hasRestaurant ? 30000 : false, // fallback polling every 30s
      refetchOnWindowFocus: hasRestaurant,
      staleTime: 0,
    }
  );

  // Real-time: refetch orders when a new order comes in or status changes
  // Join the restaurant room so the socket receives room-specific events
  useSocketEvent("new-order", () => {
    queryClient.invalidateQueries("fetchMyRestaurantOrders");
  }, restaurantId ? { restaurantId } : undefined);
  useSocketEvent("order-status-changed", () => {
    queryClient.invalidateQueries("fetchMyRestaurantOrders");
  }, restaurantId ? { restaurantId } : undefined);
